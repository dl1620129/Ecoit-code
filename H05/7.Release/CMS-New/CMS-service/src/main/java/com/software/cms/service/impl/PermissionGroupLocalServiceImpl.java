...
public class PermissionGroupLocalServiceImpl
	...
	public void doAddGroupUser(ThemeDisplay themeDisplay, long groupUserId, String groupName,
		...
		try {
			...
			group.setLanguage(themeDisplay.getLocale().toString());
			group.setGroupId(themeDisplay.getScopeGroupId());
			groupUserPersistence.update(group);
			// add user to group
			groupUserUsersPersistence.removeByGroup_GroupUser(themeDisplay.getScopeGroupId(), group.getGroupUserId());
			for (String user : users) {
				GroupUserUsersLocalServiceUtil.addGroupUserUsers(themeDisplay.getScopeGroupId(), group.getGroupUserId(), Long.parseLong(user));
			}
			// add permission to group
			permissionGroupPersistence.removeByGroup_GroupUser(themeDisplay.getScopeGroupId(), group.getGroupUserId());
			for (String permission : permissions) {
				PermissionGroupLocalServiceUtil.addPermissionGroup(themeDisplay.getScopeGroupId(), group.getGroupUserId(), Long.parseLong(permission));
			}
			...
			/*if (groupUserId <=0) addLogger("add", themeDisplay, group, users, permissions, categories,request);
			else addLogger("edit", themeDisplay, group, users, permissions, categories,request);*/
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
