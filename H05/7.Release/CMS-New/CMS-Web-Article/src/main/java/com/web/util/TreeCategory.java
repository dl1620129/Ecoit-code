...
private void recursiveCategoryTree( long parentid,String language,long groupId,long userId) throws Exception {
	...
	for (Iterator<Long> iterator = allNode.iterator(); iterator.hasNext();) {
		...
		if(node >0){
			if (PermissionUtil.checkPermissionCategory(userId, node)){
				...
				recursiveCategoryTree(node,language,groupId,userId);
			}
		}
	}
...
private void recursiveCategoryTree(long groupId, String language,long parentid,List<Long> allChild, String selected,String showCategory,int typeView,RenderRequest renderRequest,RenderResponse renderResponse) throws Exception {
	...
	for (Iterator<Long> iterator = allChild.iterator(); iterator.hasNext();) {
		...
		if((","+showCategory.trim()+",").contains(","+node+",") || showCategory.trim().equals("")){
			...
			if(typeView==0){
				...
				for (int i=0;i<5*depth;i++) result.append("&nbsp;");
				result.append(CategoryLocalServiceUtil.getCategory(node).getTitle()+"</option><br>\n");
				if(allNode!=null && allNode.size()>0)recursiveCategoryTree(groupId, language, node,allNode,selected,showCategory,typeView,renderRequest,renderResponse);
			}
			else if(typeView==1){
				if(allNode!=null && allNode.size()>0){
					...
					result.append("<img src=\""+renderRequest.getContextPath()+"/images/03_minus.png\" id=\"img_"+nodeId+"\" style=\"width: 15px; padding-top: 0px;padding-bottom: 7px;padding-right: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
		    		...
		    		result.append("/></div><div style='display: table-cell;padding-left: 5px;padding-top: 2px;position: relative;top: -2px;line-height: 20px;'>"+ CategoryLocalServiceUtil.getCategory(node).getTitle()+"</div></div></div>\n");
		    		result.append("<div id=\"div_"+nodeId+"\" >");
		    		recursiveCategoryTree(groupId, language, node,allNode,selected,showCategory,typeView,renderRequest,renderResponse);
		    		result.append("</div>");
				}
				else{
					result.append("<div id='node_"+node+"' ><div style='display: table-row;'><div style='display: table-cell;'>");
	        	...
	        	if(allNode!=null && allNode.size()>0){
	        		...
	        		for (int i=0;i<5*depth;i++) result.append("&nbsp;</div><div style='display: table-cell;'>");
	        		result.append("<img src=\""+renderRequest.getContextPath()+"/images/03_minus.png\" id=\"img_"+nodeId+"\" style=\"width: 15px; padding-top: 4px;padding-right: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
					result.append("</div><div style='display: table-cell;'><input name='checked-category' type=\"checkbox\" onchange='chkChange(this)' id=\"chk_"+nodeId+"\" value='"+node+"' ");
	        		if ((","+selected+",").contains(","+node+",")) result.append("checked='checked' ");
	        		result.append("/></div><div style='display: table-cell;padding-left: 5px;padding-top: 2px;position: relative;top: -2px;'>"+ CategoryLocalServiceUtil.getCategory(node).getTitle()+"</div></div>");
			...
			else {if(allNode!=null && allNode.size()>0)recursiveCategoryTree(groupId, language, node,allNode,selected,showCategory,typeView,renderRequest,renderResponse);}
...
private void recursiveCategoryTree(long groupId, String language,long parentid,List<Long> allChild, String selected,String showCategory,int typeView,String urlEdit,String urlDelete,HashMap hCate) throws Exception {
	List<Long> allNode = null;
	depth++;
	for (Iterator<Long> iterator = allChild.iterator(); iterator.hasNext();) {
		long node = iterator.next();
		if((","+showCategory.trim()+",").contains(","+node+",") || showCategory.trim().equals("")){
			...
			if(typeView==0){
				...
				for (int i=0;i<5*depth;i++) result.append("&nbsp;");
				result.append(CategoryLocalServiceUtil.getCategory(node).getTitle()+"</option><br>\n");
				if(allNode!=null && allNode.size()>0)recursiveCategoryTree(groupId, language, node,allNode,selected,showCategory,typeView,urlEdit,urlDelete,hCate);
			}
			else if(typeView==1){
				if(allNode!=null && allNode.size()>0){
					result.append("<div><div style='display: table-row;'><div style='display: table-cell;'>");
					for (int i=0;i<5*depth;i++) result.append("&nbsp;</div><div style='display: table-cell;'>");
					result.append("<img src=\"/images/03_minus.png\" id=\"img_"+nodeId+"\" style=\"width: 11px; padding-top: 4px;padding-right: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
					result.append("</div><div style='display: table-cell;'><input name='checked-category' type=\"checkbox\" onchange='chkChange(this)' id=\"chk_"+nodeId+"\" value='"+node+"' ");
		    		if ((","+selected+",").contains(","+node+",")) result.append("checked='checked' ");
		    		...
		    		result.append("<div id=\"div_"+nodeId+"\" >");
		    		recursiveCategoryTree(groupId, language, node,allNode,selected,showCategory,typeView,urlEdit,urlDelete,hCate);
		    		result.append("</div>");
				}
				else{
					result.append("<div><div style='display: table-row;'><div style='display: table-cell;'>");
	        	...
	        	if(allNode!=null && allNode.size()>0){
	        		...
	        		for (int i=0;i<5*depth;i++) result.append("&nbsp;</div><div style='display: table-cell;'>");
	        		result.append("<img src=\"/images/03_minus.png\" id=\"img_"+nodeId+"\" style=\"width: 11px; padding-top: 4px;padding-left: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
			...
			else {if(allNode!=null && allNode.size()>0)recursiveCategoryTree(groupId, language, node,allNode,selected,showCategory,typeView,urlEdit,urlDelete,hCate);}
			pathParent.remove(pathParent.size()-1);
		}
...
public String createCategoryTree(long[] parentCurrent,String language,long groupId,long userId) throws Exception {
...
public String createCategoryTree(long[] parentCurrent, long selected,String language,long groupId,long userId) throws Exception {
	this.parentCurrent = parentCurrent;
	List<Long> root = CategoryLocalServiceUtil.getAllChild(0, groupId, language);
	for (Iterator<Long> iterator = root.iterator(); iterator.hasNext();) {
		long node = iterator.next();
...
public String createCategoryTree(long[] parentCurrent, long groupId, String language, String selected,String showCategory,int typeView,RenderRequest renderRequest,RenderResponse renderResponse) throws Exception {
	this.parentCurrent = parentCurrent;
	List<Long> root=null;
	List<Long> allNode=null;
	long firstNode=0;
			...
			for (Iterator<Long> iterator = root.iterator(); iterator.hasNext();) {
				...
				if((","+showCategory.trim()+",").contains(","+node+",") || showCategory.trim().equals("")){
			        ...
			        if(typeView==0){
			        	...
			        	if(allNode!=null && allNode.size()>0){
			        		result.append("<div id='node_"+node+ "' ><div style='display: table-row;'><div style='display: table-cell;'>");
							result.append("<img src=\"" + renderRequest.getContextPath()+"/images/03_minus.png\" id=\"img_"+node+"\" style=\"width: 15px; padding-top: 0px;padding-bottom: 7px;padding-right: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
			        	...
			        	if(allNode!=null && allNode.size()>0){
			        		result.append("<div style='float: left;width: 100%;font-size: 18px;border-bottom: 1px solid #ececec; border-left: 1px solid #ececec;border-right: 1px solid #ececec;line-height: 30px;'><div style='display: table-row;width: 60%;float: left;'><div style='display: table-cell;'>");
							result.append("<img src=\"" + renderRequest.getContextPath()+"/images/03_minus.png\" id=\"img_"+node+"\" style=\"width: 15px; padding-top: 4px;padding-right: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
							result.append("</div><div style='display: table-cell;'><input name='checked-category' type=\"checkbox\" onchange='chkChange(this)' id=\"chk_"+node+"\" value='"+node+"' ");
			        		if ((","+selected+",").contains(","+node+",")) result.append("checked='checked' ");
			        		result.append("/></div><div style='display: table-cell;padding-left: 5px;padding-top: 2px;position: relative;top: -2px;'>"+ CategoryLocalServiceUtil.getCategory(node).getTitle()+"</div></div>");
			        		...
			        		result.append("<div style='float: left;width: 100%;font-size: 18px;border-bottom: 1px solid #ececec; border-left: 1px solid #ececec;border-right: 1px solid #ececec;line-height: 30px;'><div style='display: table-row;width: 60%;float: left;'>");
			        		result.append("<div style='display: table-cell;padding-left: 13px;'><input name='checked-category' type=\"checkbox\" onchange='chkChange(this)' id=\"chk_"+node+"\" value='"+node+"' ");
			        		if ((","+selected+",").contains(","+node+",")) result.append("checked='checked' ");
			        		result.append("/></div><div style='display: table-cell;padding-left: 5px;padding-top: 2px;position: relative;top: 3px;line-height: 20px;'>"+ CategoryLocalServiceUtil.getCategory(node).getTitle()+"</div></div>");
                            result.append("<div style='float: left;width: 25%;display: table-cell;padding-top: 2px;position: relative;top: 3px;'>"+sdf.format(CategoryLocalServiceUtil.getCategory(node).getCreatedDate())+"</div>");
...
public String createCategoryTree(long[] parentCurrent, long groupId, String language, String selected,String showCategory,int typeView,String urlEdit,String urlDelete,RenderRequest renderRequest) throws Exception {
	List<Object>lstCate=CategoryArticleLocalServiceUtil.getDuLieuTheoSql("SELECT A.categoryid,count(A.articleid) FROM soft_cms_categoryarticle A INNER JOIN soft_cms_article B ON A.articleid=B.articleid  where B.groupId = "+groupId+" AND B.language = '"+language+"' group by A.categoryid");
			...
			for (Iterator<Long> iterator = root.iterator(); iterator.hasNext();) {
				...
				if((","+showCategory.trim()+",").contains(","+node+",") || showCategory.trim().equals("")){
			        ...
			        if(typeView==0){
			        	...
			        	if(allNode!=null && allNode.size()>0){
			        		result.append("<div><div style='display: table-row;'><div style='display: table-cell;'>");
							result.append("<img src=\"/images/03_minus.png\" id=\"img_"+node+"\" style=\"width: 11px; padding-top: 4px;padding-right: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
							result.append("</div><div style='display: table-cell;'><input name='checked-category' type=\"checkbox\" onchange='chkChange(this)' id=\"chk_"+node+"\" value='"+node+"' ");
			        		if ((","+selected+",").contains(","+node+",")) result.append("checked='checked' ");
			        		result.append("/></div><div style='display: table-cell;padding-left: 5px;padding-top: 2px;position: relative;top: 3px;line-height: 20px;'>"+ CategoryLocalServiceUtil.getCategory(node).getTitle()+"</div></div></div>\n");
			        		result.append("<div id=\"div_"+node+"\" >");
			        	...
			        	if(allNode!=null && allNode.size()>0){
							...
							result.append("<img src=\"/images/03_minus.png\" id=\"img_"+node+"\" style=\"width: 11px; padding-top: 4px;padding-left: 2px;cursor: pointer;\" onclick='imgClick(this)' />");
...
public String createCategoryTree(long parentCurrent,String language,long groupId,long userId) throws Exception {
	return this.createCategoryTree(new long[]{parentCurrent},language,groupId,userId);
}

public String createCategoryTree(long parentCurrent, long selected,String language,long groupId,long userId) throws Exception {
	return this.createCategoryTree(new long[]{parentCurrent}, selected,language,groupId,userId);
}
