...
public class CMSPortalUtil {
	public static String getMinifyURL(HttpServletRequest request, String minifierType) {
...
//		String friendUrl = getFriendUrlByCategoryId(categoryId);
	...
	public static PortletURL getPortletURLFromFriendURLAndPortletId(HttpServletRequest request, Long groupId,
			long categoryId) throws PortalException, SystemException {
		String friendUrl = getFriendUrlByCategoryId(categoryId);
		Layout layout = null;
		try {
			layout = LayoutLocalServiceUtil.getFriendlyURLLayout(groupId, false, friendUrl);
		} catch (Exception e) {
			System.out.println("Tao them trang chi tiet ten: tin-chi-tiet");
		}
		return PortletURLFactoryUtil.create(request, "", layout.getPlid(), PortletRequest.RENDER_PHASE);
	}

	public static PortletURL getPortletURLFromFriendURLAndPortletIdNamePageParent(HttpServletRequest request,
			Long groupId, long categoryId, String namePageParent) throws PortalException, SystemException {
		String friendUrl = getFriendUrlByCategoryId(categoryId, namePageParent);
		Layout layout = null;
		try {
			layout = LayoutLocalServiceUtil.getFriendlyURLLayout(groupId, false, friendUrl);
		} catch (Exception e) {
	...
	public static String getFriendUrlByCategoryId(long categoryId, String namePageParent) {
		try {
	...
	public static String getUrlArticle(Article art, long catId, HttpServletRequest request, long groupId,
		...
		try {
			...
			PortletURL portletURL = getPortletURLFromFriendURLAndPortletId(request, groupId,
					Long.valueOf(catId).longValue());
