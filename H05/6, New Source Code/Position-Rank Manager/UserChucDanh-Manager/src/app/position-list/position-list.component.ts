...
@Component({
    ...
    templateUrl: 
		...
		'/o/UserChucDanh-Manager/app/position-list.component.html'
})
    ...
    selectAll(event: any) {
        ...
        if (isChecked) {
            for (let i = 0; i < this.rankModels.length; i++) {
                ...
                if(this.listId.indexOf(this.rankModels[i].userChuDanhId) == -1){
                    this.listId.push(this.rankModels[i].userChuDanhId);
                }
            }
        } else {
            for (let i = 0; i < this.rankModels.length; i++) {
                this.rankModels[i].selected = this.masterSelected;
                if (this.listId.indexOf(this.rankModels[i].userChuDanhId) !== -1) {
                    this.listId.splice(this.listId.indexOf(this.rankModels[i].userChuDanhId), 1);
                }
            }
        }
