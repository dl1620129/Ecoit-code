...
@Component({
    ...
    templateUrl: 
		...
		'/o/UserPosition-Manager/app/position-list.component.html'
})
export class PositionComponent implements OnInit{
    ...
    removedEvent: any;
    constructor(public rankservice: PositionService, public modalService:NgbModal,public notifierService: NotifierService,private  dropdownConfig:NgbDropdownConfig){
    ...
    getCategoryData(start: number){
        this.rankservice.getAllCategory(this.pageSize,start,this.searchKey).subscribe((res: any) =>{
            if(res.error_code == Constants.SUCCESSFUL_CODE){
                console.log(res);
            this.rankModels = res.data[0].userChucDanh;
    ...
    selectAll(event: any) {
        ...
        if (isChecked) {
            for (let i = 0; i < this.rankModels.length; i++) {
                ...
                if(this.listId.indexOf(this.rankModels[i].userPoisitionId) == -1){
                    this.listId.push(this.rankModels[i].userPoisitionId);
                }
            }
        } else {
            for (let i = 0; i < this.rankModels.length; i++) {
                this.rankModels[i].selected = this.masterSelected;
                if (this.listId.indexOf(this.rankModels[i].userPoisitionId) !== -1) {
                    this.listId.splice(this.listId.indexOf(this.rankModels[i].userPoisitionId), 1);
                }
            }
        }
