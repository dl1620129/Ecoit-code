...
@Component(service = ArticleFinder.class)
public class ArticleFinderImpl extends ArticleFinderBaseImpl implements ArticleFinder {
	...
	public List<Object> getDuLieuTheoSql(String sql, int start, int end) {
		...
		try {
			...
			listArt = (List<Object>) QueryUtil.list(query, getDialect(), start, end);
		} catch (Exception e) {
			listArt = null;
			// throw new SystemException(e);
		} finally {
	...
	public List<Article> findByUserNews(long groupId, String language, long userId, long status, long categoryId,
		...
		try {
			...
			if (categoryId > 0) {
				sql.append(
						" LEFT JOIN soft_cms_categoryarticle ON soft_cms_article.articleid = soft_cms_categoryarticle.articleid ");
			}
			if (userKiemDuyet) {
			...
			if (userKiemDuyet) {
				sql.append(
						" AND (LOWER( user_.firstname || ' ' ||user_.middlename || ' ' || user_.lastname ) LIKE LOWER(?) OR LOWER( user_.firstname || ' ' || user_.lastname ) LIKE LOWER(?)) ");
			}
			if (!keyword.equals("")) {
				if (checkKeywordIsNumber(keyword)) {
					sql.append(" AND soft_cms_article.articleId = " + keyword);
				} else {
					sql.append(" AND (LOWER(soft_cms_article.title) LIKE LOWER('%" + keyword
							+ "%') OR LOWER(soft_cms_article.summary) LIKE LOWER('%" + keyword
							+ "%') OR LOWER(soft_cms_article.content) LIKE LOWER('%" + keyword
							+ "%') OR LOWER(soft_cms_article.source) LIKE LOWER('%" + keyword
							+ "%') OR LOWER(soft_cms_article.events) LIKE LOWER('%" + keyword
							+ "%') OR LOWER(soft_cms_article.tags) LIKE LOWER('%" + keyword
							+ "%') OR LOWER(soft_cms_article.author) LIKE LOWER('%" + keyword + "%'))");
				}
			}
			...
			if (status == NewsWorkflowConstants.STATUS_DRAFT) {
				...
				if (status == NewsWorkflowConstants.STATUS_PENDING) {
					if ((PermissionUtil.checkPermissionAccessZone(userId, RoleConstants.ARTICLE_PENDING, groupId))
							&& (!PermissionUtil.checkPermissionAccessZone(userId, RoleConstants.VIEW_ARTICLE,
									groupId))) {
						sqlOr = " soft_cms_article.humanaccessids LIKE '%" + userId + "%'";
					}
				}
				if (!PermissionUtil.checkPermissionAccessZone(userId, RoleConstants.VIEW_ARTICLE, groupId)) {
					if (sqlOr.equals("")) {
						sqlOr = " soft_cms_article.createdbyuser = ? ";
					} else {
			...
			if (!sqlOr.isEmpty())
				sql.append(" AND (" + sqlOr + ")");
			if (status == NewsWorkflowConstants.STATUS_DRAFT || status == NewsWorkflowConstants.STATUS_PENDING) {
				sql.append(" ORDER BY soft_cms_article.createddate DESC ");
			}
			if (status == NewsWorkflowConstants.STATUS_PUBLISH || status == NewsWorkflowConstants.STATUS_PUBLISH_DENIED) {
				sql.append(" ORDER BY soft_cms_article.publisheddatefrom DESC ");
			}
			if (status == NewsWorkflowConstants.STATUS_APPROVED || status == NewsWorkflowConstants.STATUS_APPROVE_DENIED) {
				sql.append(" ORDER BY soft_cms_article.approveddate DESC ");
			}
			/*if (status == NewsWorkflowConstants.STATUS_PENDING || status == NewsWorkflowConstants.STATUS_PUBLISH_DENIED
			...
			result = (List<Article>) QueryUtil.list(query, getDialect(), start, end);
	...
	public int getCountByUserNews(long groupId, String language, long userId, long status, long categoryId,
		...
		try {
			...
			sql.append(" WHERE soft_cms_article.groupid = ? " + " AND soft_cms_article.language = ? "
	...
	public int getCountArticleByCaterory(long groupId, Long categoryId, Date publishDateFrom) throws SystemException {
		...
		String sql = "SELECT COUNT(DISTINCT soft_cms_article.articleid) FROM soft_cms_article "
				...
				+ " WHERE soft_cms_categoryarticle.categoryid = ?" + " and soft_cms_article.groupid = ? " // + groupId
		...
		if (publishDateFrom != null) {
			...
			sql += " AND soft_cms_article.publisheddatefrom <= '" + sdf.format(publishDateFrom) + "' ";
		}
		try {
			session = openSession();
			SQLQuery q = session.createSQLQuery(sql);
	...
	public List<Article> getListArticleByCaterory(long groupId, Long categoryId, Date publishDateFrom, Integer start,
			Integer end) throws SystemException {
		...
		try {
			...
			String sql = "SELECT DISTINCT soft_cms_article.* FROM soft_cms_article INNER JOIN soft_cms_categoryarticle "
					+ "ON soft_cms_article.articleid=soft_cms_categoryarticle.articleid "
					+ "WHERE soft_cms_categoryarticle.categoryid = ? " + " and soft_cms_article.groupid = ? " // +
																												// groupId
					+ " AND soft_cms_article.status= " + NewsWorkflowConstants.STATUS_PUBLISH;
			if (publishDateFrom != null) {
				Calendar c = Calendar.getInstance();
			...
			listArt = (List<Article>) QueryUtil.list(query, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		} finally {
			closeSession(session);
	...
	public List<Article> getListArticleByEvent(long groupId, long articleId, long categoryId, String eventId,
			Integer start, Integer end) throws SystemException {
		List<Article> listArt = new ArrayList<Article>();
		Session session = null;
		try {
			session = openSession();
			...
			sql += " AND soft_cms_categoryarticle.articleid != " + articleId + " AND soft_cms_article.status= "
					+ NewsWorkflowConstants.STATUS_PUBLISH + " ORDER BY publisheddatefrom DESC";
			SQLQuery query = session.createSQLQuery(sql);
			query.setCacheable(true);
			query.addEntity("Article", ArticleImpl.class);
			QueryPos qPos = QueryPos.getInstance(query);
