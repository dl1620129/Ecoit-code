...
public class ArticleLocalServiceImpl extends ArticleLocalServiceBaseImpl {
	...
	public List<Article> getArticleFeaturedNew(long groupId, long categoryId, int thutu, int limit, String locale,
			boolean ishomepage, boolean isnoibat, int status, String lIds) {
		List<Article> danhSachArticle = null;
		Session session = null;
		try {
			session = this.articlePersistence.openSession();
			String sql = "select * from soft_cms_article where groupid=" + groupId + " and language='" + locale + "' "
					+ " and status=" + status;
	...
	public List<Article> searchNewArticle(long groupId, int status, String keyword, String keytag, boolean key,
			boolean tag, int start, int limit) throws Exception {
		List<Article> danhSachArticle = null;
		Session session = null;
		try {
			session = this.articlePersistence.openSession();
			...
			String sql = "";
			if (key && !tag)
				sql += "SELECT * FROM soft_cms_article  WHERE groupid =" + groupId + " and status=" + status
						+ " AND (lower(title) LIKE LOWER('%" + keyword + "%') OR LOWER(summary) like LOWER('%" + keyword
						+ "%')) ORDER BY publisheddatefrom DESC";
			if (tag && !key) {
				sql += "SELECT * FROM soft_cms_article  WHERE groupid =" + groupId + " and status=" + status
						+ " AND LOWER(tags) LIKE LOWER('%" + keytag + "%') ORDER BY publisheddatefrom DESC";
	...
	public int countSearchNewArticle(long groupId, int status, String keyword, String keytag, boolean key, boolean tag)
			throws Exception {
		int count = 0;
		Session session = null;
		try {
			...
			String sql = "";
			if (key && !tag)
				sql += "SELECT count(*) FROM soft_cms_article  WHERE groupid =" + groupId + " and status=" + status
						+ " AND (lower(title) LIKE LOWER('%" + keyword + "%') OR LOWER(summary) like LOWER('%" + keyword
						+ "%'))";
			if (tag && !key)
				sql += "SELECT count(*) FROM soft_cms_article  WHERE groupid =" + groupId + " and status=" + status
						+ " AND lower(tags) LIKE LOWER('%" + keytag + "%')";
	...
	public int getCountByUserNews(long groupId, String language, long userId, long status, long categoryId,
			String eventName, String tagName, String keyword, boolean inTitle, boolean inSummary, boolean inContent,
			boolean inDate, Date fromDate, Date toDate, boolean inAuthor, boolean userKiemDuyet)
			throws Exception {
