 ...
 * <code>com.software.cms.service.TagLocalService</code> interface.
 *
 * <p>
 ...
 */
@Component(property = "model.class.name=com.software.cms.model.Tag", service = AopService.class)
public class TagLocalServiceImpl extends TagLocalServiceBaseImpl {
	 ...
	 * <code>com.software.cms.service.TagLocalService</code> via injection or a
	 * <code>org.osgi.util.tracker.ServiceTracker</code> or use
	 * <code>com.software.cms.service.TagLocalServiceUtil</code>.
	 */
	public List<Tag> getListTag(String name, long groupId, String language, int start, int limit) throws Exception {
		List<Tag> listTag = new ArrayList<Tag>();
		Session session = null;
		try {
			session = this.tagPersistence.openSession();
			String sql = "select * from soft_cms_tag where groupid=" + groupId + " and language='" + language + "'";
			if (!name.isEmpty()) {
				sql += " AND LOWER(tagname) LIKE LOWER('%" + name + "%') ";
			}
			if (start != -1 && limit != -1) {
	...
	public int getCountByTag(String name, long groupId, String language) throws Exception {
		int total = -1;
		Session session = null;
		try {
			session = this.tagPersistence.openSession();
			String sql = "select count(*) from soft_cms_tag where groupid=" + groupId + " and language='" + language + "'";
			if (!name.isEmpty()) {
				sql += " AND LOWER(tagname) LIKE LOWER('%" + name + "%') ";
			}
			SQLQuery query = session.createSQLQuery(sql);
	...
	public List<Tag> searchByTagName(String name, long groupid, String language) {
		...
		try {
			session = this.tagPersistence.openSession();
			String sql = "select * from soft_cms_tag where groupid = " + groupid + " AND language = '" + language
					+ "' AND  LOWER(tagname) like LOWER('%" + name + "%') limit 50";
			SQLQuery query = session.createSQLQuery(sql);
			query.setCacheable(false);
			query.addEntity("Tag", TagImpl.class);
			listTag = (List<Tag>) query.list();
	...
	public int getCountByTagName(long groupId, String language, String tagname) {
		...
		try {
			...
			sql.append(" WHERE soft_cms_tag.groupid = " + groupId + " AND soft_cms_tag.language = '" + language + "'"
