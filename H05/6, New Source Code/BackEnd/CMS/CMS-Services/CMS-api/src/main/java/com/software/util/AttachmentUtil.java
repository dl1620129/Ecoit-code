...
public class AttachmentUtil {
	...
	public static long uploadFileResize(UploadPortletRequest upload,ThemeDisplay themeDisplay,Article article,File file, String title, int w, int h ){
		long entryId = 0;
		try {
	...
	public static long ResizeImage(UploadPortletRequest upload,ThemeDisplay themeDisplay,Article article,File file, String title, int w, int h ){
		long entryId = 0;
		try {
			File fileDes = new File(PortalUtil.getPortalWebDir()+"image_resize/"+title);
			//String end = title.substring(title.indexOf(".")+1);
