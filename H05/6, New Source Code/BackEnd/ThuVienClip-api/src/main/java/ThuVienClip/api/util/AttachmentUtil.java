...
public class AttachmentUtil {
	 ...
	 * @return id of file in repository( return 0 if no file upload or upload
	 *         invalid)
	 * @throws SystemException
	 * @throws PortalException
	 */

	public static long uploadFile(UploadServletRequest upload, Clip clip, File file, String title,
			...
			if (clip.getFolderId() <= 0) {
				...
				if (dlFolder == null) {
					dlFolder = DLFolderLocalServiceUtil.addFolder(userId, scopeGroupId, scopeGroupId, false,
							folderMonth.getFolderId(), "Video_" + clip.getId(), clip.getTitle(),
							false, serviceContext);

				}

				clip.setFolderId(dlFolder.getFolderId());
			} else {
				try {
					dlFolder = DLFolderLocalServiceUtil.getDLFolder(clip.getFolderId());
				} catch (Exception e) {
					dlFolder = null;
				}
				if (dlFolder == null) {
...
//			e.printStackTrace();
	...
	public static long uploadFileResize(UploadPortletRequest upload,Clip  clip, File file, String title, int w,
			int h, long scopeGroupId, long userId) {
		long entryId = 0;
		try {
			...
			DLFolder dlFolder = null;
			if (clip.getFolderId() <= 0) {
				dlFolder = DLFolderLocalServiceUtil.addFolder(userId, scopeGroupId, scopeGroupId, false, 0,
						"Video_" + clip.getId(), clip.getTitle(), false, serviceContext);
				clip.setFolderId(dlFolder.getFolderId());
			} else
				dlFolder = DLFolderLocalServiceUtil.getDLFolder(clip.getFolderId());
			FileEntry image = DLAppLocalServiceUtil.addFileEntry(userId, scopeGroupId, dlFolder.getFolderId(), title,
					mimeType, title, "", "", fileDes, serviceContext);
...
//	public static long ResizeImage(UploadPortletRequest upload, Clip clip, File file, String title, int w, int h,
...
//
//				BufferedImage reImg = Scalr.resize(img, Mode.AUTOMATIC, (int) outer.getWidth(),
//						(int) outer.getHeight());
//
...
//						reImg = reImg.getSubimage((int) x, (int) y, (int) copySize.getWidth(),
//								(int) copySize.getHeight());
//						size = copySize;
//					} else
//						size = outer;
...
//				}
//				// draw
//				BufferedImage bufferedResizedImage = new BufferedImage((int) Math.round(copySize.getWidth()),
//						(int) Math.round(copySize.getHeight()), BufferedImage.TYPE_INT_RGB);
...
//				dlFolder = DLFolderLocalServiceUtil.addFolder(userId, scopeGroupId, scopeGroupId, false, 0,
//						"Video_" + article.getArticleId(), article.getTitle(), false, serviceContext);
