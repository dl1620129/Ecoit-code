...
public interface IFileEntryRepository extends IRepository<FileEntry> {
	...
	@Query("Select file from FileEntry file where (:clientId is null or  file.clientId = :clientId) and file.active=:active and file.name=:name ORDER BY file.name ASC")
	List<FileEntry> findByName(@Param(value = "clientId") Long clientId,@Param(value = "active") Boolean active,@Param(value = "name") String name); 
