...
import com.ecodoc.backend.core.repository.IStraceSystemRepository;
import com.ecodoc.backend.util.DateTimeUtils;
...
public class StraceSystemService {
	...
	private String getClientName() {
		try {
			return InetAddress.getByName(request.getRemoteAddr()).getHostName();
	...
	public ListObjectDto<StraceDto> search(String userName, Date startDate, Date endDate, Long idCat, Pageable page) {
		Page<StraceDto> pageRs = iStraceSystemRepository.search(userName, startDate, endDate, idCat, BussinessCommon.getClientId(), page);
	...
	public ListObjectDto<StraceDto> search(String userName, Date startDate, Date endDate, Long idCat, Sort sort) {
		List<StraceDto> listRs = iStraceSystemRepository.search(userName, startDate, endDate, idCat, BussinessCommon.getClientId(), sort);
	...
	public ListObjectDto<StraceAdminDto> searchAdmin(Date startDate, Date endDate, Long idCat, Integer month,
		...
		Page<StraceAdminDto> pageRs = iStraceSystemRepository.searchAdmin(startDate, endDate, idCat, frMonth, toMonth, month, year, quarterly, pageable);;
		return BussinessCommon.paging(StraceAdminDto.convert(pageRs));
	}
	
//	public ListObjectDto<StraceDocDto> searchDoc(Date startDate, Date endDate, Integer month, Integer year, Integer quarterly, Long docType, DocumentStatusEnum status, Pageable page) {
//		int toMonth = DateTimeUtils.getMonthFromQuarterly(quarterly, 1);
//		int frMonth = DateTimeUtils.getMonthFromQuarterly(quarterly, 2);
//		Page<StraceDocDto> pageRs = iStraceSystemRepository.searchDoc(frMonth, toMonth, CategoryEnum.DOCUMENT.getValue() , startDate, endDate, month, year, quarterly, docType, status, page);
