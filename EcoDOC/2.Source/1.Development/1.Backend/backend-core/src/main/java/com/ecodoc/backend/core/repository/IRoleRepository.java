...
public interface IRoleRepository extends IRepository<Role> {
	...
	boolean existUserIdByClientIdAndNamesInUserRole(
			@Param("userId") Long userId, @Param("clientId") Long clientId, @Param("roleNames") String[] roleNames);
	...
	List<Role> getRoleHaveModuleByUserAndActiveAndClientId(String code, long userId, long posId, boolean active, Long clientId);
}
