...
export class UserDetailComponent implements OnInit {
  ...
  doUploadFile() {
    return this.uploadFileService.uploadFile(this.userAvatar).subscribe(
      event => {
        this.user.photo = event.message;
        this.toastr.success('Tải ảnh cá nhân thành công', 'Thành công');
      });
  }
  ...
  goBack() {
    const navigationExtras: NavigationExtras = {state: {action: "back"}};
  ...
  getToken(){
    this.tokenInfoService.getTokenInfo((data) => {
      if (data !== '') {
        const tokenInfo = JSON.parse(this.tokenInfoService.b64DecodeUnicode(data));
        const arr: any = tokenInfo.Subject.split(",");
        this.user.nameToken = tokenInfo.CommonName;
        this.user.serialToken = tokenInfo.SerialNumber;
        this.user.startTimeToken = tokenInfo.ValidDate;
        this.user.expiredTimeToken = tokenInfo.ExpireDate;
        // this.CA.orgToken = tokenInfo.Subject.substring(tokenInfo.Subject.lastIndexOf(',') + 1).trim();
        let org = arr[arr.length-2].trim();
        this.user.orgToken = org.substr(2,org.length);
      }
    });
  }
